$(document).ready(function() {
   jQuery(function(){	
      ...
      jQuery("#posts div.post div.inside").click(function() {
         console.log('clicked on div.inside');
      ...
      $("#posts_big img.next").click(function() {
         // get the trend
         var trend = $('#posts_big').attr('data-trend');
         // get the current tweet unique ID
         var id = $('#posts_big').attr('data-trend-id');
         // only keep the number
         var id = parseInt(id.replace(/post_(\d+)/, '$1'));

         // find all tweets with this trend
         var tweets = $('#posts').find($('.'+ trend));
         ...
         var nexthtml = $('#' + foo[0] + ' span.tweet').html();

         // change the tweet message
         if(nexthtml !== ''){
            $('#posts_big span.big_quote').html(nexthtml);
         }else{
            $('#posts_big span.big_quote').html("No more tweets, try again later.");
         }
         ...
         // var next = $('#posts').find($(this).parent.attr('data-trend'));
         // console.log(this);
         // var decodeURIComponent()
         // var $parent = $(this).parent("div.big_post"); 
      ...
      $('#posts_big img.previous').click(function() {
         // get the trend
         var trend = $('#posts_big').attr('data-trend');
         // get the current tweet unique ID
         var id = $('#posts_big').attr('data-trend-id');
